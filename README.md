...
Risk Management Dashboard MVP for Finalto. This project demonstrates a real-time dashboard for
visualizing key metrics relevant to book management, including PnL curve, monetization, client yield,
and PnL attribution.
...
This will generate `dist/test_finalto-1.0.0.tar.gz` and `dist/test_finalto-1.0.0-py3-none-any.whl`.
...
Mypy looks for the existence of a file named [`py.typed`](./src/test_finalto/py.typed) in the root of the
...
1. Calling `python` in the project root (for example, `python -m pytest tests/`)
   ...
   package `test_finalto` is in the project root, it shadows the `test_finalto` package installed in the Nox
   session.
...
```shell
docker build --tag test-finalto .
```

To run the image in a container:

```shell
docker run --rm --interactive --tty test-finalto
```
